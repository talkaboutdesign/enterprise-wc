...
class MaskAPI {
  ...
  conformToMask(rawValue: string, maskObj: any, settings: any) {
     ...
     */
    function markAddedChars(char: any, j: any) {
      return {
        char,
  ...
  adjustCaretPosition(opts: any) {
     ...
     */
    function nonPlaceholderFilter(char: any) {
    ...
    if (possiblyHasRejectedChar) {
      ...
      const intersection = leftHalfChars.filter((char: any) => normalizedConformedValue.indexOf(char) !== -1); //eslint-disable-line
      ...
      if (opts.indexesOfPipedChars) {
        pipedChars = opts.indexesOfPipedChars.map((index: any) => normalizedConformedValue[index]);
      }

      // We need to know how many times the `targetChar` occurs in the piped characters.
      const countTargetCharInPipedChars = pipedChars.filter((char: any) => char === targetChar).length;

      // We need to know how many times it occurs in the intersection
      const countTargetCharInIntersection = intersection.filter((char: any) => char === targetChar).length; // eslint-disable-line
      ...
      const countTargetCharInPlaceholder = opts.placeholder
        ...
        .filter((char: any, index: any) => char === targetChar && opts.rawValue[index] !== char).length;
   ...
   */
  getSafeRawValue(inputValue: any) {
